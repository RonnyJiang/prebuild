...
def build(path):
    ...
    cmd = "docker exec -u pre_build:pre_build %s sh -c 'cd %s && ./%s sdk_root_rel=%s'" % (get_running_containerid(), path, sys.argv[1], sys.argv[4])
    if sys.argv[4].find(get_env("aurora_sdk")) >= 0:
        download_and_upzip_sdk(sys.argv[4], get_env("aurora_build_sdk_url"))
        cmd = "docker exec -u pre_build:pre_build %s sh -c 'cd %s && ./%s'" % (get_running_containerid(), path, sys.argv[1])
    LOG.info(cmd)
