...
def scp_from_server(ip, user, password, server_file, local_dir, server_file_is_dir=False):
...
def reset_code(path, error_datas):
    ...
    for row in error_datas:
        ...
        LOG.info('reset_code item: %s,and current dir: %s' % (row, os.getcwd()))
    ...
    if status == 0:
        for project_path in project_paths:
            ...
            if change.find("Untracked files:") >=0 or change.find('use "git push" to publish your local commits') >= 0 or change.find("nothing to commit") == -1:
                LOG.info('access git clean directory,the change is %s,git dir is %s' % (change, project_path))
