...
"""
 ...
 @contact: set@aliyun.com
 @site: www.lemon.pub
 @software: PyCharm  @since:python 3.5.2(32bit) on 2016/11/30.14:35
"""
import logging
import os
import sqlite3
...
def get_project_path(path, project):
    ...
    if status == 0:
        for project_path in project_paths:
            os.chdir(path + "/" + project_path + "/..")   #eg: cd /home/ronny/workspace/aurora_app/packages/app/ChannelManagerService/.git/..
...
def copy_xml_file():#生成一个关于各个build.sh src和dest对应的dict
    copy_list = {}
    ...
    if status == 0:
        ...
        for project_node_item in project_node:
            ...
            for copy_node_item in copy_node:
                ...
                ##copy_list[device/s3graphics/zx2000/build.zx2000.sh]=build.sh
                ##copy_list[device/s3graphics/ncn_evb/build.sh]=build.ncn_evb.sh
                ##copy_list[device/s3graphics/ocn_evb/build.sh]=build.ocn_evb.sh
                ##copy_list[device/s3graphics/zx2000/setbuildenv]=setbuildenv
                ##copy_list[device/s3graphics/zx2000/flash.sh]=flash.sh
                ##copy_list[device/s3graphics/core/root.mk]=Makefile
                ##copy_list[device/s3graphics/build.sh]=build.stb_sdk.sh
                ##copy_list[device/s3graphics/ocn_sva/build.sh]=build.ocn_sva.sh  ---and so on
    ...
    # copy_xml_file = {'device/s3graphics/ocn_lj/build.sh': 'build.ocn_lj.sh', 'device/s3graphics/zx2000/setbuildenv': 'setbuildenv',
    #  'device/s3graphics/ncn_sva/build.sh': 'build.ncn_sva.sh', 'device/s3graphics/zx2000/build.zx2000.sh': 'build.sh',
    #  'device/s3graphics/ocn_sva/build.sh': 'build.ocn_sva.sh', 'device/s3graphics/zx2000/flash.sh': 'flash.sh',
    # 'device/s3graphics/ocn_evb/build.sh': 'build.ocn_evb.sh', 'device/s3graphics/ncn_evb/build.sh': 'build.ncn_evb.sh',
    # 'device/lt/stb_sdk/build.sh': 'build.stb_sdk.sh', 'build/core/root.mk': 'Makefile', 'device/s3graphics/ncn_lj/build.sh': 'build.ncn_lj.sh'}
    return copy_list


def repo_copy_files(path): #将所有build.sh从xml中标识的src拷贝到path位置，且在拷贝之前删掉旧的build file---即更新build脚本，赋予可执行权限
    ...
    for (src, dest) in copy_list.items():
        try:
            ...
            #copy 后给build脚本777权限 user ：rwx ， Group： rwx ，Other： rwx
        except Exception, e:
            LOG.error(e)
...
def build(path, clean_build):
    try:
        ...
        if clean_build and os.path.exists(out_path):
            shutil.rmtree(out_path)   #shutil.rmtree( src ) 递归删除一个目录以及目录内的所有内容
            LOG.info('start clean build...')

        cmd = "docker exec -u pre_build:pre_build %s sh -c 'cd %s && ./%s'" % (
        get_running_containerid(), path, sys.argv[5])
        LOG.info(cmd)
...
def post_build(dataServer, issue_id, base_path):
    ...
    # errot_datas =  [(u'venus/tvos/device/lt/stb_sdk', u'refs/changes/03/118603/1', u'112797', u'TankZhou@www.com')]
    if error_datas is False or len(error_datas) == 0:
        LOG.error("error_datas is Null")
        return
    clean_build = 0
    for row in error_datas:
        project_name = row[db_schema.project_name]   #venus/tvos/device/lt/stb_sdk
        ref_spec = row[db_schema.ref_spec]    #refs/changes/03/118603/1
        patch_set = ref_spec.split('/')[-2]   #118603
        clean_build += get_clean_build(patch_set)
        run_gerrit_command(project_name, "0", 'Build Start ' + get_env('BUILD_URL') + 'console',
                           ','.join(ref_spec.split('/')[-2:]))
        #/home/pre_build/workspace/pre_build_aurora_slave---本地build目录  u'venus/tvos/device/lt/stb_sdk', u'refs/changes/03/118603/1'
    ...
    if build_result:
        for row in error_datas:
            ...
            ref_spec = ','.join(row[db_schema.ref_spec].split('/')[-2:])  #118603
...
def main():
    LOG.info('main in')
    #python /home/pre_build/workspace/AutoTest/pre-build/pre_build_slave.py +
    # + /home/pre_build/workspace/pre_build_aurora_master 10.27.16.108 pre_build 123456 build.sh liantong/aurora 10.27.254.106:8888
    # 即：get_env("WORKSPACE") 为/home/pre_build/workspace/pre_build_aurora_slave---本地build目录
    #    sys.argv[1] 为/home/pre_build/workspace/pre_build_aurora_master，远程master server目录
    #    sys.argv[2]为10.27.16.108---远程master server IP
    #    sys.argv[3]为pre_build --远程master server login ID
    #    sys.argv[4]为123456   --远程master server password
    #    sys.argv[5]为build.sh  编译脚本
    #    sys.argv[6]为liantong/aurora  docker
    #    sys.argv[7]为10.27.254.106:8888
