#!/usr/bin/env python
# -*- coding: utf-8 -*-
# lt.py command groups main python files
#

import sys

if sys.hexversion < 0x02070000:
    print >> sys.stderr, "Python 2.7 or newer is required."   #echo Python 2.7 or newer is required.
    sys.exit(1)
...
def _command(cmd, env = None, stderr = None, stdout = None, cwd = None):
    ...
    cmds = cmd if not isinstance(cmd, basestring) else shlex.split(cmd)
    p = subprocess.Popen(cmds, stderr = stderr, stdout = stdout, cwd = cwd)
    p.communicate()
    return p.returncode
...
def _fetch_tools(local, quiet):
    ...
    if proc.wait() != 0:
        print("error: could not sync tools. \nPlease run 'sudo lt.py clean' comamnd and try again!")
        raise CloneFailure()
...
def _clone_tools(local):
    ...
    proc = subprocess.Popen(shlex.split(cmds), stdout = LT_OPTIONS[LOGGER_FD],\
            stderr = LT_OPTIONS[LOGGER_FD], cwd = local)
    if proc.wait() != 0:
        print("error: could not down tools")
        raise CloneFailure()
    else:
        _fix_permisson(local)
    ...
    if mfile is not None and os.path.exists(mfile):
        ...
        if _md5sum(ofile) != _md5sum(mfile):
            try:
                ...
                print("error: No permission update command\n"\
...
def _clean_tools(mfile, workdir):
    if mfile is not None and workdir is not None \
        ...
        dst = os.path.join(os.getenv('HOME'), TOOLS_CONFIG)
        if dst == os.path.dirname(purgedir):
            _fix_permisson(dst)
...
def main(args):
    ...
    try:
        ...
        print("error: unable to start %s" % cmd)
        sys.exit(148)
