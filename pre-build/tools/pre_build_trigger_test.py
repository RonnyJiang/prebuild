...
"""
 ...
 @site: www.lemon.pub
 @software: PyCharm  @since:python 3.5.2(32bit) on 2016/11/25.13:31
"""
...
import os
import logging
import subprocess  ##py2 use commmands
import sys
import re
...
def get_profile_list(content):
    ...
    for index,one_xml in enumerate(total_xml):
        ...
        print('---namegroup----\n','<xml>:',result.group('xml'),'\n<config_path>',result.group('trigger_config_path'))
        profile_list[result.group('xml')] = result.group('trigger_config_path')
...
project_list = {}
def parse_xml(xml_file, project_list, branch='master'):
    os.chdir(get_env('PYDIR'))
    xml_path = os.path.join(get_env('PYDIR'),xml_file)
    print(xml_path)
    ...
    for include_node_item in include_node:
        parse_xml(include_node_item.get('name'),project_list,default_branch)
    return project_list
parse_xml('apps_aurora.xml',project_list)
